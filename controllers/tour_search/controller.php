    ...
    {  
        ...
        */
        ...
        if(!empty($wp->request)) {
            ...
            $url_arr = explode("/", $wp->request);

            if(isset($url_arr[0]) && strtolower($url_arr[0]) == "things-to-do") {
                if(count($url_arr) > 2) {
                    $current_slug = $url_arr[2];
                    $this->country_name = $url_arr[1];
        ...
        //$DIR_REL = GRAYLINE_LICENSEE_WORDPRESS_TOURCMS_PLUGIN_PATH;
        ...
        if (!empty($_GET['reload'])) {
            ...
            if (!isset($search_params["location"]) && !isset($search_params["country_name"]) && !isset($search_params["search_term"])) {
                if (isset($_GET['from_tour'])) {
                    $url = home_url("?q=".$_GET["from_tour"]."&searchInput=1");
                    header("Location: $url");
                }
            } else {
            ...
            ($current_slug != "") ? $search_params["location"] = $current_slug : null;
            ...
            isset($_GET['page']) ? $search_params["page"] = (int)$_GET['page'] : $search_params["page"] = 1;
        ...
        if($search_params['country_name'] == 'korea--republic-of') {
            $search_params['country_name'] = null;
            $search_params['country'] = 'KR';
        }
        if ($search_params['location'] == 'Korea  Republic of') {
